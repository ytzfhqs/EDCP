...
class BaseChat:
    ...
    @staticmethod
    def ex_score_and_reason(model_res: str, prompt_type: str) -> Tuple[str, int]:
        if prompt_type == "domain":
            reason: str
            score_str: str
            score: int
...
class ChatGLM4(BaseChat):
    ...
    def do_process(
        ...
        reason, score = self.ex_score_and_reason(response.choices[0].message.content, prompt_type)
